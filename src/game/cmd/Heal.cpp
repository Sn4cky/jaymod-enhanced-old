...
Heal::doExecute( Context& txt )
{
    ...
    if (txt._args.size() > 2) {
        numHeal = atoi( txt._args[2].c_str() );
        if (numHeal < 1)
            numHeal = 1;
    ...
    if (single) {
       ...
       buf << xvalue( u.namex ) << " was healed.";
    }
    else
       buf << xvalue( num ) << " player" << (num == 1 ? "" : "s") << " healed.";
